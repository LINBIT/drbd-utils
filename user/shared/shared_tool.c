...
/* #include <w34api/winbase.h>
 * just gives lots of conflicts... */
extern uint32_t QueryDosDeviceA(const char* lpDeviceName, char* lpTargetPath, uint32_t ucchMax);

char *WindowsResolveDeviceLink(char *input)
{
    long ret;
    char destination[256];

    ret = QueryDosDeviceA(input, destination, sizeof(destination)-1);
    if (ret <= 0)
        return NULL;
 ...
 *   (/dev/)?sdXN
 *   X:
 *   (Volume{)?d4f04fbd-9bf7-11e6-b442-525400092996}?
 * this returns a (newly allocated) string of the form
 *   \Device\HarddiskVolume<N>
 * which can be understood by humans and the NT kernel as well.
 * */
char * WindowsLowLevelDeviceName(char *input)
{
    int harddisk, partition;
    int len, ret;
    char drive_letter[2];
	char name[256];

    ret = sscanf(input, "Harddisk%dPartition%d%n", &harddisk, &partition, &len);
    if (ret == 2 && input[len] == 0) {
        return WindowsResolveDeviceLink(input);
    }

    ret = sscanf(input, "/dev/sd%1[a-zA-Z]%d%n", drive_letter, &partition, &len);
    if (ret == 0)
        ret = sscanf(input, "sd%1[a-zA-Z]%d%n", drive_letter, &partition, &len);
    if (ret == 2 && input[len] == 0) {
        harddisk = toupper(drive_letter[0]) - 'A';
        ret = snprintf(name, sizeof(name),
                 "Harddisk%dPartition%d",
                harddisk, partition);
		name[sizeof(name)-1] = 0;

        return WindowsResolveDeviceLink(name);
    }

    if (isalpha(input[0]) && input[1] == ':' && input[2] == 0)
    {
        /* Windows drive letter, like "X:".
         * Perhaps we should allow for two- (and more?) character drive specifications
         * as well; while "explorer.exe" won't show them, the symlinks to the drivers
         * work just fine. */
        return WindowsResolveDeviceLink(input);
    }

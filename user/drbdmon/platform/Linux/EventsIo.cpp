...
const int EventsIo::WAKEUP_CTL_INDEX = 4;
// Number of epoll_event datastructure slots
const int EventsIo::CTL_SLOTS_COUNT  = 5;
...
EventsIo::EventsIo(const int events_input_fd, const int events_error_fd):
    ...
    input_seq_dec(new InputSequenceDecoder()),
    ...
    discard_buffer(new char[DISCARD_BUFFER_SIZE])
{
    try
    {
        ...
        if (pipe2(wakeup_fd, 0) != 0)
        {
            std::string error_msg("Unable to setup an internal communication pipe");
            std::string debug_info("pipe2(...) failed, errno=");
            debug_info += std::to_string(errno);
            throw EventsIoException(&error_msg, &debug_info, nullptr);
        }

        // Make the wakeup pipe nonblocking
        {
            const int read_io_flags = fcntl(wakeup_fd[posix::PIPE_READ_SIDE], F_GETFL, 0);
            fcntl(wakeup_fd[posix::PIPE_READ_SIDE], F_SETFL, read_io_flags | O_NONBLOCK);
            const int write_io_flags = fcntl(wakeup_fd[posix::PIPE_WRITE_SIDE], F_GETFL, 0);
            fcntl(wakeup_fd[posix::PIPE_WRITE_SIDE], F_SETFL, write_io_flags | O_NONBLOCK);
        }
        ...
        register_poll(wakeup_fd[posix::PIPE_READ_SIDE], &(ctl_events[WAKEUP_CTL_INDEX]), EPOLLIN);
    }
...
void EventsIo::wakeup_wait() noexcept
{
    // Full pipe, etc. are ignored, because the only purpose of the pipe is to wake up the epoll wait,
    // which is going to happen anyway if there is something in the pipe already
    errno = 0;
    int rc = 0;
    do
    {
...
// @throws std::bad_alloc, EventsIoException
CoreIo::event EventsIo::wait_event()
{
    ...
    while (event_id == EventsIo::event::NONE)
    {
        ...
        if (fired_fd == stdin_fd)
        {
            // Data available on stdin
            {
                ...
                {
                    errno = 0;
                    read_count = read(STDIN_FILENO, &in_key, 1);
                }
                while (read_count == -1 && errno == EINTR);

                if (read_count == 1)
                {
                    key = input_seq_dec->input_char(in_key, *mouse);
        ...
        if (fired_fd == wakeup_fd[posix::PIPE_READ_SIDE])
        {
            // Events thread wakeup
            // Discard data in the wakeup pipe
            char* const discard_buffer_ptr = discard_buffer.get();
            ...
            while ((read_count == -1 && errno == EINTR) || read_count >= 1);
            if (initial_read && read_count == 0)
            {
                std::string error_msg("The internal wakeup pipe stream was closed");
                throw EventsIoException(&error_msg, nullptr, nullptr);
            }
            else
            if (read_count == -1)
            {
                if (errno != EAGAIN)
                {
                    std::string error_msg("Reading from the internal wakeup pipe stream failed");
                    std::string debug_info("read(wakeup_fd[posix::PIPE_READ_SIZE], ...) failed, errno=");
                    debug_info += std::to_string(errno);
                    throw EventsIoException(&error_msg, &debug_info, nullptr);
                }
            }
            event_id = EventsIo::event::WAKEUP;
        }
        else
        {
...
// @throws std::bad_alloc, EventsIoException
CoreIo::signal_type EventsIo::get_signal()
{
    int sys_signal_id = 0;
    ...
    do
    {
        ...
        {
            sys_signal_id = static_cast<int> (signal_buffer->ssi_signo);
        }
        else
        if (read_size == 0)
        {
    ...
    CoreIo::signal_type dm_signal = CoreIo::signal_type::SIGNAL_GENERIC;
    switch (sys_signal_id)
    {
        case SIGHUP:
            // fall-through
        case SIGINT:
            // fall-through
        case SIGTERM:
            dm_signal = CoreIo::signal_type::SIGNAL_EXIT;
            break;
        case SIGWINCH:
            dm_signal = CoreIo::signal_type::SIGNAL_WINDOW_SIZE;
            break;
        case SIGCHLD:
            dm_signal = CoreIo::signal_type::SIGNAL_CHILD_PROC;
            break;
        case SIGUSR1:
            dm_signal = CoreIo::signal_type::SIGNAL_DEBUG;
            break;
        case SIGALRM:
            dm_signal = CoreIo::signal_type::SIGNAL_TIMER;
        default:
...
uint32_t EventsIo::get_key_pressed()
{
    return key;
}

MouseEvent& EventsIo::get_mouse_action()
{
    return *mouse;
}
...
void EventsIo::cleanup() noexcept
{
    ...
    posix::close_fd(wakeup_fd[posix::PIPE_READ_SIDE]);
    posix::close_fd(wakeup_fd[posix::PIPE_WRITE_SIDE]);

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <!-- Package name. -->
 <xsl:param name="package">drbd-pacemaker</xsl:param>

 <!-- Package version number. Must be passed in. -->
 <xsl:param name="version"></xsl:param>

 <!-- these entries are used for the refentry source name profile -->
 <xsl:param name="orgname">LINBIT HA Solutions GmbH</xsl:param>

 <!-- RA class -->
 <xsl:param name="class">ocf</xsl:param>

 <!-- RA provider -->
 <xsl:param name="provider">linbit</xsl:param>
 ...
 <xsl:template match="resource-agent" mode="refentryinfo">
   <refentryinfo>
     ...
     <!--
     <corpname><xsl:value-of select="$corpname"/></corpname>
     <corpcredit><xsl:value-of select="$corpcredit"/></corpcredit>
     <corpauthor><xsl:value-of select="$corpauthor"/></corpauthor>
     <publishername><xsl:value-of select="$publishername"/></publishername>
	-->
   </refentryinfo>
 </xsl:template>
      ...
      <para>
        <ulink>
          <xsl:attribute name="url">
            <xsl:text>https://docs.linbit.com/</xsl:text>
          </xsl:attribute>
        </ulink>
	<!-- someone please tell me how to do a proper "list" instead of manually inserting blanks here -->
	<xsl:text>, </xsl:text>
        <ulink>
          <xsl:attribute name="url">
            <xsl:text>https://clusterlabs.org/</xsl:text>
          </xsl:attribute>
        </ulink>
	<xsl:text>, </xsl:text>
        <ulink>
          <xsl:attribute name="url">
            <xsl:text>https://www.linbit.com/drbd-community/</xsl:text>
          </xsl:attribute>
        </ulink>
      </para>
    </refsection>
